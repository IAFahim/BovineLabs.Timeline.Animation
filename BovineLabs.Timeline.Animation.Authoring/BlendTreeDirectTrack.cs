    ...
    public class BlendTreeDirectTrack : DOTSTrack
    {
        ...
        public override Playable CreateTrackMixer(PlayableGraph graph, GameObject go, int inputCount)
        {
            if (!Application.isPlaying)
            {
                ...
                // Stamp each clip with a back-ref to this track so BlendTreeDirectClip.CreatePlayable can reach Motions
                // and preview the highest-weight motion instead of an empty bind pose.
                foreach (var c in GetClips())
                    if (c.asset is BlendTreeDirectClip btc)
        ...
        protected override void Bake(BakingContext context)
        {
            ...
            {
                Debug.LogWarning(
                    $"[BlendTreeDirectTrack] '{name}' rig '{rigDef.name}' has no Avatar — animation data will not be baked.");
                base.Bake(context);
                return;
            }
